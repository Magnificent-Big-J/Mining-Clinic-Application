<div class="row">
    <div class="col-lg-6">
        <div class="form-group row">
            <label class="col-lg-3 col-form-label">Street:<i class="fa fa-asterisk text-danger"></i></label>
            <div class="col-lg-9">
                <input type="text" name="street"  class="form-control">
                <small id="street-error" class="text-danger"></small>
            </div>
        </div>
        <div class="form-group row">
            <label class="col-lg-3 col-form-label">Complex:<i class="fa fa-asterisk text-danger"></i></label>
            <div class="col-lg-9">
                <input type="text" name="complex"  class="form-control">
                <small id="complex-error" class="text-danger"></small>
            </div>
        </div>
        <div class="form-group row">
            <label class="col-lg-3 col-form-label">Suburb:<i class="fa fa-asterisk text-danger"></i></label>
            <div class="col-lg-9">
                <input type="text" name="suburb" id="suburb" class="form-control">
                <small id="suburb-error" class="text-danger"></small>
            </div>
        </div>

    </div>
    <div class="col-lg-6">

        <div class="form-group row">
            <label class="col-lg-3 col-form-label">City:<i class="fa fa-asterisk text-danger"></i></label>
            <div class="col-lg-9">
                <input type="text" name="city" class="form-control" required>
                <small id="city-error" class="text-danger"></small>
            </div>
        </div>
        <div class="form-group row">
            <label class="col-lg-3 col-form-label">Postal Code:<i class="fa fa-asterisk text-danger"></i></label>
            <div class="col-lg-9">
                <input type="text" name="postal_code" class="form-control" required>
                <small id="postal-code-error" class="text-danger"></small>
            </div>
        </div>

    </div>
</div>
